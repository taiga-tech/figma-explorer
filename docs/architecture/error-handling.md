...
`src/utils/result.ts`（予定）に定義し、ヘルパー（`ok()` / `err()`）を併設する。
...
| storage | 読み込み失敗・破損時はバックアップキーへ退避してから初期状態を返す |
| 移行    | migration 失敗時は元データを退避し、読み取り専用の警告表示にする   |
